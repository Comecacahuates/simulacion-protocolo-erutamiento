...
class RoutingProtocolBase  : public inet::RoutingProtocolBase, public omnetpp::cListener, public inet::NetfilterBase::HookBase  {
...
protected:
    ...
    // Acks
    virtual const inet::Ptr<Ack> createAck(const inet::Ipv6Address &address) const;
    virtual void sendAck(const inet::Ptr<Ack> &ack, const inet::Ipv6Address &destinationAddress);
    ...
    inet::Ipv6Address getClosestNeighbouringCarAddress(const GeohashLocation &geohashLocation) const;

    // Routes
    virtual void addRoute(const inet::Ipv6Address &destPrefix, const short prefixLength, const inet::Ipv6Address &nextHop, int metric, omnetpp::simtime_t expiryTime);
    ...
    // Routing
    virtual inet::INetfilter::IHook::Result routeDatagram(inet::Packet *datagram, const inet::Ipv6Address &destAddress) = 0;

    // TLV options
    void setTlvOption(inet::Packet *datagram, inet::TlvOptionBase *tlvOption) const;
    template <class T> const T *findTlvOption(inet::Packet *datagram) const {
        ...
        inet::Ptr<const inet::Ipv6Header> ipv6Header = inet::dynamicPtrCast<const inet::Ipv6Header>(inet::getNetworkProtocolHeader(datagram));
        const inet::Ipv6ExtensionHeader *extensionHeader = ipv6Header->findExtensionHeaderByType(inet::IpProtocolId::IP_PROT_IPv6EXT_HOP);
        const inet::Ipv6HopByHopOptionsHeader *optionsHeader = omnetpp::check_and_cast_nullable<const inet::Ipv6HopByHopOptionsHeader *>(extensionHeader);
    ...
    template <class T> T *findTlvOptionForUpdate(inet::Packet *datagram) {
        ...
        T *tlvOption = nullptr;
        inet::Ptr<inet::Ipv6Header> ipv6Header = inet::constPtrCast<inet::Ipv6Header>(inet::dynamicPtrCast<const inet::Ipv6Header>(inet::getNetworkProtocolHeader(datagram)));
        ...
        if (optionsHeader) {
            ...
            int i = 0;
            while (i < tlvOptions.getTlvOptionArraySize()) {
                tlvOption = dynamic_cast<T *>(tlvOptions.getTlvOptionForUpdate(i++));
    ...
    TlvDestGeohashLocationOption *createTlvDestGeohashLocationOption(uint64_t geohashLocationBits) const;
    int computeTlvOptionLength(TlvDestGeohashLocationOption *tlvOption) const;

    // TLV destination on road network location option
    TlvDestLocationOnRoadNetworkOption *createTlvDestLocationOnRoadNetworkOption(Vertex vertexA, Vertex vertexB, double distanceToVertexA) const;
    void setTlvDestLocationOnRoadNetworkOption(inet::Packet *datagram, const GeohashLocation &destGeohashLocation) const;
    int computeTlvOptionLength(TlvDestLocationOnRoadNetworkOption *tlvOption) const;
    ...
    // Netfilter
    virtual inet::INetfilter::IHook::Result datagramPreRoutingHook(inet::Packet *datagram) override { return inet::INetfilter::IHook::ACCEPT; }
    ...
    // Notification
    virtual void receiveSignal(omnetpp::cComponent *source, omnetpp::simsignal_t signalID, omnetpp::cObject *obj, cObject *details) override = 0;
};
